...
class RRTStarDubins(RRTStar):
    ...
    def __init__(self, start, goal, obstacle_list, obstacles, rand_area,
                 goal_sample_rate=10,
                 max_iter=1000,
    ...
    def planning(self, animation=True, search_until_max_iter=True):
        ...
        for i in range(self.max_iter):
            ...
            if self.check_collision(new_node, self.obstacle_list, self.obstacles):
    ...
    def draw_graph(self, rnd=None):
        ...
        for (ox, oy, width, height) in self.obstacles:
            boxes_with_margin = plt.Rectangle((ox, oy), width, height, fc='white',ec="red")
            margin = 0.25
            boxes = plt.Rectangle((ox+margin, oy+margin), width-2*margin, height-2*margin, fc='black',ec="black")
            plt.gca().add_patch(boxes_with_margin)
            plt.gca().add_patch(boxes)
    ...
    def generate_final_course(self, goal_index):
        ...
        for i in range(len(path)-1):
            dist += np.sqrt((path[i+1][1]-path[i][1])**2 + (path[i+1][0]-path[i][0])**2 )
        print('distance of final path =', dist)
...
def main():
    ...
    obstacles_simple, obstacles_medium, obstacles_hard, start, goal, rand_area = maps.generate_maps()
    ...
    rrtstar_dubins = RRTStarDubins(start = start, goal = goal, rand_area=rand_area, obstacle_list=obstacleList, obstacles = obstacles)
